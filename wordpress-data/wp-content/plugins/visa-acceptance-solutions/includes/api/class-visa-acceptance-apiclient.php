 ...
 * @subpackage Visa_Acceptance_Solutions/includes
 */
class Visa_Acceptance_ApiClient {
	...
	public function service_processor( $payload, $resource_data, $service_header, $service, $settings, $log_header ) {
		// Adding condition to avoid empty Authorisation request.
		$flex_url = VISA_ACCEPTANCE_FLEX_TEST_DOMAIN;
	...
	private function get_base64_encode( $input_string ) {
		return base64_encode( $input_string );// phpcs:ignore
	}
	...
	private function get_base64_decode( $input_string ) {
		return base64_decode( $input_string );//phpcs:ignore 
	}
	...
	private function get_environment_settings( $settings ) {
		...
		if ( VISA_ACCEPTANCE_ENVIRONMENT_TEST === ( ! empty( $settings['environment'] ) ? $settings['environment'] : '' ) ) {
			$general_settings['merchant_id']         = ( ! empty( $settings['test_merchant_id'] ) ? $settings['test_merchant_id'] : '' );
			$general_settings['merchant_key_id']     = ( ! empty( $settings['test_api_key'] ) ? $settings['test_api_key'] : '' );
			$general_settings['merchant_secret_key'] = ( ! empty( $settings['test_api_shared_secret'] ) ? $settings['test_api_shared_secret'] : '' );
		} else {
			$general_settings['merchant_id']         = ( ! empty( $settings['merchant_id'] ) ? $settings['merchant_id'] : '' );
			$general_settings['merchant_key_id']     = ( ! empty( $settings['api_key'] ) ? $settings['api_key'] : '' );
			$general_settings['merchant_secret_key'] = ( ! empty( $settings['api_shared_secret'] ) ? $settings['api_shared_secret'] : '' );
		}
